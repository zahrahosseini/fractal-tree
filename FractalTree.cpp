...
//glTranslatef(0.0f,0.0f,0.0f); baraye inke bere markazesh..
	glTranslatef(_x,_y,_z-3.0f);// bere tahe cylinder
...
// mige marhale ba'di chand bashe.
static void step(){
	turtle_x+=0.200000000f* cos((turtle_theta)*0.017453292f);
	turtle_y+=0.200000000f* sin((turtle_theta)*0.017453292f);
}
static void FT( float _x,float _y,float _Width,float _Height,float angle ,int level){
	...
	if(level<3){
		...
		drawcylander(angle,_Width,distance(_x,_y,x1,y01),_x,_y,0);
	}
	else
	{
	...
	/*	turtle_theta=angle;
		turn(Langle);*/
		FT(x1,y01,_Width,_Height,(Langle+angle),level);
		/*turtle_theta=theta(_x,_y,x1,y01);
		turn(Rangle);*/
		FT(x1,y01,LWfactor*_Width,LHfactor*_Height,(angle-Rangle),level);
	}
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glColor3f(0.0,0.0,0.0);
	level=Level_in;
	LWfactor=pow(2.0f,-1.0f/Lalpha);
	RWfactor=pow(2.0f,-1.0f/Ralpha);
	LHfactor=pow(2.0f,-2.0f/(3*Lalpha));
	Rhfactor=pow(2.0f,-2.0f/(3*Ralpha));
	x=0;
	y=0;
	x1=0.0f;
	y01=0.2f;
	drawcylander(0.0f,0.1f,0.2f,x,y,0);
	turtle_r=0.2f;
	turtle_theta=theta(x,y,x1,y01);
	turtle_x=x;
	turtle_y=y;
	turn(Langle);
	FT(x1,y01,0.1f,0.2f,Langle,level);
	turtle_theta=theta(x,y,x1,y01);
	turn(Rangle);
	FT(x1,y01,0.1f,0.2f,-Rangle,level);
	glFlush();							
	return TRUE;										
}
